  ...
  that.setState({ isLoading: true, hasLoadedTasks: false, errorLoadingTasks: false,
    ...
    fetch(url) // basic fetch without options does a GET
      ...
      .then(obj => {
        // Happy case
        let loadedTasksList;
console.log('returned from get ', obj)
        // If empty tasks list or data not format expected, create empty list
        ...
        that.setState({ tasks: loadedTasksList, hasLoadedTasks: true }, () => {
          console.log("Loaded data: ", loadedTasksList, that.state.tasks);
        });
      })
...
export const setTasks = (url, data, that) => {
  ...
  console.log('going to try to set ', data);
    fetch(url, {
      method: "post",
